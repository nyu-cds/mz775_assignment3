...
def zbits(n, k):
    ...
    binary_list = ''.join(list(it.repeat('1',(n-k))) + list(it.repeat('0',k))) # create a binary list that consists of strings 1 and 0 
    permutations = list(it.permutations(binary_list, n)) # generate the permutation 
    reformat_permutations = [''.join(list(i)) for i in permutations] # change format of permutation, for example change '1','1','0' to '110'
